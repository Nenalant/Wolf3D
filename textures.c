...
void	init_weapon(t_env *e)
{
	...
	e->sabre.img = mlx_xpm_file_to_image(e->mlx, sniper, &e->s_weight, &e->s_height);
	e->sabre.data = mlx_get_data_addr(e->sabre.img, &e->sabre.bpp, &e->sabre.size_line, &e->sabre.endian);
}
...
void	init_sky(t_env *e)
{
	...
	e->etoile.data = mlx_get_data_addr(e->etoile.img, &e->etoile.bpp, &e->etoile.size_line, &e->etoile.endian);
}
...
void	put_weapon(t_env *e)
{
	...
	while (i < e->s_height)
	{
		...
		while (j < e->s_weight)
		{
			pos_i = 4 * (W - e->s_weight + j) + e->size_line * (H - e->s_height + i);
			pos_sabre = 4 * j + e->sabre.size_line * i;
			if (e->data[pos_sabre] != 0 && e->data[pos_sabre + 1] != 0 && e->data[pos_sabre + 2] != 0)
			{
