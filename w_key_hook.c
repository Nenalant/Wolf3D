...
void	left_and_right(int key, t_env *e)
{
	...
	if (key == LEFT || key == Q)
	{
		...
		e->player_dirx = e->player_dirx * cos(e->rot_speed) - e->player_diry * sin(e->rot_speed);
		e->player_diry = old_dir_x * sin(e->rot_speed) + e->player_diry * cos(e->rot_speed);
		old_plane_x = e->player_cam_x;
		e->player_cam_x = e->player_cam_x * cos(e->rot_speed) - e->player_cam_y * sin(e->rot_speed);
		e->player_cam_y = old_plane_x * sin(e->rot_speed) + e->player_cam_y * cos(e->rot_speed);
	}
...
int		key_hook(int key, t_env *e)
{
	if (key == 53)
		clean(e);
	if ((key >= 123 && key <= 126) || (key >= 0 && key <= 2) || (key == 13) || (key == 6))
	{
